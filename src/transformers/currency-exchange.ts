...
const currencyExchange = (exchangeService: ExchangeService, targetCurrency: Currency, defaultSourceCurrency?: Currency) =>
  transform<TransactionCombinedAmount, Promise<TransactionCombinedAmount>>(
    ...
    async (data: TransactionCombinedAmount, cb, params) => {
      ...
      const newMemo = `${data.amount.toLocaleString()} ${data.currency || defaultSourceCurrency} (FX rate: ${exchangeRate.toPrecision(4)})${
        data.memo ? ` | ${data.memo}` : ''
      }`;
